...
function thumbnailClick(newImgPath, sel, tx, xx) {
	thClickTx = tx;
...
function saveKalendoscopeImage() {
...
function saveKalendoscopeImWebp() {
	var ca = $('textGif').value != '' ? newCanAddTx(canvas) : canvas;
...
function readFile(file, org) {
	...
	promise.then(function (result) {
		if (url !== null && url !== undefined) {
			var dc = img.src, w = img.width, h = img.height;
			if (w > 2500 || h > 2500) {
				...
				var her = new Hermite_class(); her.resample(tc, w, h, true, function () {
					dc = tc.toDataURL();
...
function startR(e) {
	var file, allfil;
	try {
		if (e.target.files || e.files || e.dataTransfer.files) {
			allfil = (e.target.files || e.files || e.dataTransfer.files);
			if (allfil.length) {
				for (var i = 0; i < allfil.length; i++) {
...
function clipIchNew() {
	...
	this.init = function () {
		pastdiv = document.createElement('div'); pastdiv.setAttribute('id', 'pasteff');
			...
			var observer = new MutationObserver(function (mut) {
				mut.forEach(function (m) {
					...
					if (m.addedNodes.length == 1) {
						if (m.addedNodes[0].src != undefined) {
						...
						setTimeout(function () { pasteEsup = true }, 20)
					} else { return false }
				});
			});
	...
	this.on_keyup = function (event) {
		if (!txt_akt()) {
			if (event.ctrlKey == false && ctrl_pr == true) { ctrl_pr = false }
...
function drawTileToCanvas(scale) {
	var offset = 0.6;
...
function describeArc(x, y, radius, startAngle, endAngle) {
	var pc = function (cX, cY, r, ang) {
		var an = (ang - 90) * Math.PI / 180.0;
...
function addImageToImagesUL(uli, url, tx) {
	...
	if (uli.id == 'imagesUl') {
		if (localStorage.getItem('startimg') && localStorage.getItem('startimg') == 'set') { }
			...
			ulli = uli.appendChild(document.createElement('li'));
...
function clFieldsets(ii) {
	if ($('KaleidosSp').clientHeight < 500) {
...
function handleMousemove(e, can, sel) {
	if (can.isDrawing || (e.target.id == 'svgUseShape' && e.target.isDrawing)) {
...
function eventOutAlls() {
	$('svgdiv').isDrawing = false; setmouseov = 1;
	$('svgUseShape').isDrawing = false; $('textCanvas').isDrawing = false;
}
	...
	try { currentTiling.draw() } catch (err) {
		if (err.name === 'NS_ERROR_NOT_AVAILABLE') {
			window.setTimeout(function () {
				updateScreenElemsSize(); changeCSS()
			}, 300)
		} else { }
	}
}
...
function changeCSSin(ii) {
	if (ii == 2) {
		setMouseSelColor = document.getElementsByName('transpset')[0].value * 1 > 0 ? 1 : 0;
...
function transpset() {
	var trv = document.getElementsByName('transpset')[0].value * 1;
	setMouseSelColor = trv > 0 ? 1 : 0; transpsets[3] = trv;
	$('textCanvas').className = setMouseSelColor ? 'kaleidoCanvasCr' : 'kaleidoCanvasCs';
}

function vignetteSet() {
	canvasVignette = document.getElementsByName('vignetteset')[0].value * 1;
...
function setTranspColors(ct, w, h) {
	transpset();
...
function setNewFonts() {
	if ($('stylefonts')) { }
		...
		var cssff = '@font-face{font-weight:normal;font-style:normal;font-display:swap;}',
...
function saveFonts() {
	return [$('textGif').value, $('gifTxColor').value, $('gifTxColor2').value, $('gifFonts').value, $('gifTxSize').value * 1, $('gifTxBoKs').value, $('gifTxPos').value]
}
function gifShareSet() { if ($('gifImages').style.display != 'block') { textCanvasGp(); saveAppToDb(0, saveFonts(), 16) } }
function gifFontsSet() {
	if ($('gifFonts').value == 'arial ') { setNewFonts() }
...
function textCanSet(ca, tx, ps, w, h) {
	if (tx != '') {
...
function buildGIF(w, h, imgs, set) {
	...
	if (set < 1) { textCanSet(imgs, tx_v, poscor, w, h); gif.addFrame(imgs) } else {
		var txx = '', imln = arr.length;
		for (var i = 0; i < imln; i++) {
			txx += (tx_va.length > 1 && i < tx_va.length ? txva[i] + '<br>' : (i < txva.length ? txva[i] + ' ' : ''));
	...
	gif.on('finished', function (blob) {
		...
		if (set < 1) { $('allgifdone').innerHTML = 'New GIF Image (' + w + 'x' + h + 'px size: ' + fileSizeKbMb(blob[1] / 1000) + ')' }
			...
			$('allgifsdone').innerHTML = 'Animated GIF (' + w + 'x' + h + 'px size: ' + fileSizeKbMb(blob[1] / 1000) + ') ' +
...
function newGifKaleido() {
	$('shareAsPng').disabled = true;
...
function buildAniGIF() {
	$('gifImages').style.display = 'block';
	...
	const i = setInterval(function () {
		...
		if (ct == lng) { clearInterval(i); buildGIF(canvasWidthHeight[0], canvasWidthHeight[1], 1, lng); }
	}, 50);
}
...
function setAppMoment() {
	var allda = [[1, 0, 1], [1, 0, 2], [2, 0.4, 3], [3, 12, 4], [1, 1, 5], [2, 1, 6], [3, 1, 7], [4, 0, 8], [5, 0, 9], [1, 0, 10], [2, 0, 11], [3, [0, 0, 0], 12],
...
function resetAllHist() {
	txHistIns = 0; UndoRedoPos = 0; namesUndoRedo = [];
...
function undoHist(pp) {
	var his = 1; txHistIns = 0;
	...
	if (UndoRedoPos > 2 && namesUndoRedo[UndoRedoPos - 2][1].length > 20) {
		openDesignsSv(namesUndoRedo[UndoRedoPos - 2][1]); UndoRedoPos--
	} else {
		for (var i = UndoRedoPos - 2; i >= 0; i--) {
			if (namesUndoRedo[i][2] == namesUndoRedo[UndoRedoPos - 1][2]) {
...
function setSavedHist() {
	var myAr; txHistIns = 0;
	localforage.getItem('UndoRedoPos').then((val) => {
		myAr = val || 0; UndoRedoPos = myAr;
		localforage.getItem('allDataBefore').then((val) => {
			myAr = val || []; allDataBefore = myAr;
			localforage.getItem('namesUndoRedo').then((val) => {
				myAr = val || []; namesUndoRedo = myAr;
				txHistIns = 0; showNames();
				for (var i = 1; i < namesUndoRedo.length; i++) {
					var abc = namesUndoRedo[i];
...
function closeAPosDiv() {
	$('ownDesignsDiv').style.display = 'none';
	$('allImages').style.display = 'none'; $('undoredoDiv').style.display = 'none';
}

function designsSel() {
	ownDesiseAct = ownDesiseAct == 0 ? 1 : 0;
...
function designsSDel() {
	if (ownDesiseAct && ownDesisel.length > 0) {
		...
		if (navigator.setAppBadge) {
			navigator.setAppBadge(ownDesigns.length);
...
function openDesigns(dd) {
	if (ownDesiseAct) {
		var set = ownDesisel.indexOf(dd) > -1 ? 1 : 0;
		...
		var d = ownDesigns[dd]; txHistIns = 0;
		for (var i = 0; i < 18; i++) { setoAppFunc(d[i], 1); } setoAppFunc(d[0], 1);
		txHistIns = 1; saveAppToDb(1, d, 23)
	}
}

function allDesigns(ems) {
	closeAPosDiv(); ownDesisel = []; ownDesiseAct = 0;
	localforage.getItem('ownDesigns').then((val) => {
		...
		if (ownDesigns.length > 0) {
			$('ownDesignsbt').disabled = false;
			$('ownDesignsbt').innerHTML = 'Own Designs<k class="appbadge">' + ownDesigns.length + '</k>';
			if (navigator.setAppBadge) { navigator.setAppBadge(ownDesigns.length); }
			for (var i = 0; i < ownDesigns.length; i++) {
				var abc = ownDesigns[i];
...
function addDesign() {
	...
	var her = new Hermite_class(); her.resample(c, w, h, true, function () {
		...
		setTimeout(function () {
			$('addDesignVi').style.opacity = 0; $('addDesignVi').style.padding = '0';
			$('addDesignVi').style.maxWidth = 0; $('ownDesignsbt').disabled = false;
		}, 2500);
	})
}
	...
	try {
		...
		Object.keys(fonts)
			...
			.forEach((fontFamily, index) => {
				fonts[fontFamily]
					.forEach((font) => {
						var fo = font.fullName.replace(' Normal', ''), fontna = '\'' + fo + '\'',
...
function imageTrEyed() {
	...
	eyeDropper.open().then((result) => {
		...
		if (testHColor(c)) {
			$('setRemColor').value = c; valueTransp();
